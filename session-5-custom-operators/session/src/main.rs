...
impl<G: Scope> SessionWindow<G> for Stream<G, (char, Action)> {
    fn sessionize(&self, epoch_timeout: usize) -> Stream<G, Vec<Action>> {

        self.unary_frontier(Pipeline, "SessionWindow", |cap, info| {
            let mut vector = Vec::new();
            let mut notificator = FrontierNotificator::new();
            let mut stash = HashMap::new();  // session_id -> list of events
            ...
            // TODO another approach: only maintain session_id_to_closing_time, and go through it on notification
            move |input, output| {
                while let Some((time, data)) = input.next() {
                    // let (session_id, user_interaction) = data.clone();
                    data.swap(&mut vector);
                    for (session_id, user_interaction) in vector.drain(..) {
                        stash.entry(session_id.clone())
                             .or_insert(Vec::new())
                             .push(data.replace(Vec::new()));
                        // remove old closing time
                        if let Some(closing_time) = session_id_to_closing_time.get(&session_id) {
                            if let Some(session_ids) = closing_time_to_session_ids.get(closing_time) {
                                session_ids.retain(|&x| x != session_id); // .remove(session_id);
                            }
                        }
                        // update closing time
                        let notification_time = time.time() as usize + epoch_timeout;
                        session_id_to_closing_time[&session_id] = notification_time;
                        closing_time_to_session_ids.entry(notification_time)
                                                   .or_insert(Vec::new())
                                                   .push(session_id);
                        notificator.notify_at(cap.delayed(&G::Timestamp::from_integer(notification_time)));
                    }
                }
                // when notified
                while let Some((time, count)) = notificator.next() {
                    println!("time {:?} complete with count {:?}!", time, count);
                    // TODO: custom window action function
                    ...
                    if let Some(session_ids) = closing_time_to_session_ids.remove(time.time()) {
                        for session_id in session_ids.into_iter() {
                            ...
                            if let Some(list) = stash.remove(session_id) {
